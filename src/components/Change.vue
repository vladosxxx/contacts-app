  ...
  <div>
    ...
    <ul>
      <li v-for="(fieldLocal, index) in fieldLocals" v-bind:key="index">
        ...
        >
        <div>
        <button v-on:click="fieldCancel(fieldLocal)">Отменить</button>
        <button v-on:click="delField(index)">Удалить</button>
        </div>
      </li>
    </ul>
    ...
    <button v-on:click="addField()">Добавить поле</button>
    </div>
    <br>
    <div class="bottom-buttons">
      <hr>
      <button v-on:click="saveContact()">Сохранить и выйти</button>
    <button v-on:click="cancel()">Отменить и выйти</button>
    </div>
...
export default {
  ...
  methods:{
    ...
    editField(fieldEditing){ // Редактирование поля
